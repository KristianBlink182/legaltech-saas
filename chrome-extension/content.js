console.log("JUDIBOT Extension v2.0: Puente Directo Activo.");

// 1. SI ESTAMOS EN LA WEB DE JUDIBOT (VERCEL):
// La extensión inyecta automáticamente todos los expedientes guardados al Dashboard
if (window.location.href.includes("legaltech-saas") || window.location.href.includes("localhost")) {
  chrome.storage.local.get(["judibot_saved_cases"], (res) => {
    if (res.judibot_saved_cases && Array.isArray(res.judibot_saved_cases) && res.judibot_saved_cases.length > 0) {
      localStorage.setItem("judibot_cases", JSON.stringify(res.judibot_saved_cases));
      console.log("JUDIBOT: Expedientes sincronizados directamente al Dashboard.");
    }
  });
}
...
function inyectarBotonesJUDIBOT() {
  ...
  // BOTÓN 2: SINCRONIZAR ESTE EXPEDIENTE REAL
